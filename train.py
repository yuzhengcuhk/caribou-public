...
def run(seed:     Annotated[int,   ArgInfo(help='initial random seed')] = 12345,
    ...
    for iteration in range(repeats):
        start_time = time()
        #print("debugging...", data_initial, "debugging...")
        data = Data(**data_initial.to_dict())
        # print("debugging...METHOD", method, "debugging...")
        metrics = method.fit(data)
        end_time = time()
        # print("debugging...", metrics, "debugging...")
...
def main():
    ...
    try:
        ...
        if torch.cuda.is_available():
            ...
            gpu_mem_mb = gpu_mem * 1024
            console.info(f'Max GPU memory used = {gpu_mem:.2f} GB ({gpu_mem_mb:.2f} MB)\n')
