...
SCRIPT_DIR="AE/OV/scripts"

# Directory for output files (will be created if it doesn't exist)
OUTPUT_DIR="AE_outputs/OV/ove"

# Create output directory if it doesn't exist
mkdir -p "$OUTPUT_DIR"

# Counter for tracking progress
total=$(find "$SCRIPT_DIR" -maxdepth 1 -name "*.sh" | wc -l)
current=0
...
for script in "$SCRIPT_DIR"/*.sh; do
    ...
    # Run only if the script name contains "edp"
    if [[ $script_name == *"edp"* ]]; then
        # Output file path
