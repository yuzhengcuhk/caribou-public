...
class GAP (NodeClassification):
    ...
    def fit(self, data: Data, learn: str = 'transductive', prefix: str = '') -> Metrics:
        ...
        if learn == 'transductive':
            ...
            return super().fit(self.data, learn, prefix=prefix)
            ...
            # Add virtual node
            # data_add_vn = VirtualNode()
            # data = data_add_vn.forward(data)
    ...
    def _normalize(self, x: torch.Tensor) -> torch.Tensor:
        # feature-level clip
        feature_clip_value = 1
        feature_norm = torch.norm(x, p=2, dim=0)
        clipped_feature = torch.clamp(feature_norm, min=0, max=5)
        x = torch.div(x, clipped_feature) 

        # node-level clip
        node_norm = torch.norm(x, p=2, dim=-1)
        clipped_node_norm = torch.max(node_norm, torch.ones_like(node_norm))
        min_norm = torch.min(clipped_node_norm)
        x = x / min_norm
    ...
    def compute_aggregations(self, data: Data) -> Data:
        with console.status('computing aggregations'):
            ...
            for k in range(self.hops):
                ...
                ### GAP with skip links
                #if (len(x_list)<2):
                x = x + x_list[0] +  x_list[-1]
                #else:
                #    x = x + x_list[-2] + x_list[-1]

                x = x + torch.mean(x[-1]) #x_list[-1]
                x_list.append(x)
                '''
                x = self._normalize(x)  # GAP
                # x = 
                x_list.append(x)
