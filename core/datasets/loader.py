...
#from torch_geometric.datasets import Chains
...
from core.datasets.chains import Chains
...
class DatasetLoader:
    supported_datasets = {
        ...
        'chains': partial(Chains, name='Chains',
            transform=None
        ),
        'chains1': partial(Chains, name='chain-1',
            transform=None
        ),
        'chains2': partial(Chains, name='chain-2',
            transform=None
        ),
        'chains3': partial(Chains, name='chain-3',
            transform=None
        ),
        'chains4': partial(Chains, name='chain-4',
            transform=None
        ),
        'chains5': partial(Chains, name='chain-5',
            transform=None
        ),
    }
    ...
    def __init__(self,
        ...
        print("DatasetLoader", self.name, self.data_dir)
    ...
    def load(self, verbose=False, csbm_phi=0.0) -> Data:
        ...
        if self.name == 'chains':
            data = Compose([ToSparseTensor()])(data)
            pass
        elif 'chains' in self.name:
            data = Compose([ToSparseTensor()])(data)
        else:
            data = Compose([RemoveSelfLoops(), RemoveIsolatedNodes(), ToSparseTensor()])(data)
        
        # print("Data loaded", data)
        # print("Data loaded", data.x)
        # print("Data verbose", verbose) 
    ...
    def print_stats(self, data: Data):
        ...
        # print("debug-----print_stats", data.train_mask)
